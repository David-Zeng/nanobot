...
class AgentLoop:
    ...
    def _register_default_tools(self) -> None:
        ...
        for cls in (GlobTool, GrepTool):
            ...
            if tool.name not in skip:
                self.tools.register(tool)
        notebook_tool = NotebookEditTool(workspace=self.workspace, allowed_dir=allowed_dir)
        if notebook_tool.name not in skip:
            self.tools.register(notebook_tool)
        if self.exec_config.enable and "exec" not in skip:
            self.tools.register(
                ExecTool(
    ...
    async def _run_agent_loop(
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        async def _drain_pending(*, limit: int = _MAX_INJECTIONS_PER_TURN) -> list[dict[str, Any]]:
            ...
            async def after_iteration(self, context: AgentHookContext) -> None:
                # Inject graceful warning when 1 tool call remains before the limit.
                if context.iteration == loop_self.max_iterations - 1 and context.tool_results:
                    warning = (
                        f"\n\n[System: 1 tool call remaining before the iteration limit "
                        f"({loop_self.max_iterations}). Finalise your task now — write any "
                        "result files and update status fields before your next response. "
                        "Do not make further tool calls after this one.]"
                    )
                    # Append warning to the last tool result message in context.
                    msgs = context.messages
                    for i in range(len(msgs) - 1, -1, -1):
                        if msgs[i].get("role") == "tool":
                            content = msgs[i].get("content", "")
                            if isinstance(content, str):
                                msgs[i] = {**msgs[i], "content": content + warning}
                            break
        ...
        if result.final_content:
            result.final_content = ContextBuilder.strip_runtime_context(result.final_content)
